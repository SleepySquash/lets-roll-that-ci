...
FCM_PROJECT = $(or $(FCM_PROJECT_ID),lets-roll-that-ci-3872c)
FCM_BUNDLE = $(or $(FCM_BUNDLE_ID),com.sleepysquash.lets-roll-that-ci)
...
appcast.item:
	@echo "<item><title>$(appcast-item-ver)</title>$(if $(call eq,$(notes),),$(appcast-item-notes),<description>$(notes)</description>)<pubDate>$(shell date -R)</pubDate>$(call appcast.item.release,"macos","lets-roll-that-ci-macos.zip")$(call appcast.item.release,"windows","lets-roll-that-ci-windows.zip")$(call appcast.item.release,"linux","lets-roll-that-ci-linux.zip")$(call appcast.item.release,"android","lets-roll-that-ci-android.zip")$(call appcast.item.release,"ios","lets-roll-that-ci-ios.zip")</item>" \
...
clean.test.e2e:
	rm -rf .dart_tool/build/generated/lets-roll-that-ci/integration_test \
...
minikube-mount-pid = $(word 1,$(shell ps | grep -v grep \
                                         | grep 'minikube mount' \
                                         | grep 'sleepysquash-lets-roll-that-ci'))
...
helm-chart := $(or $(chart),lets-roll-that-ci)
...
helm-release = $(call slugify,40,\
	lets-roll-that-ci$(strip $(if $(call eq,$(helm-cluster),staging),,\
...
#
# Usage:
#	make helm.lint [chart=lets-roll-that-ci]
...
#
# Usage:
#	make helm.package [chart=lets-roll-that-ci]
...
#
# Usage:
#	make helm.release [chart=lets-roll-that-ci]
...
ifeq ($(minikube-mount-pid),)
	minikube mount "$(PWD):/mount/sleepysquash-lets-roll-that-ci" &
endif
endif
